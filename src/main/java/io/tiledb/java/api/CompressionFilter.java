...
public class CompressionFilter extends Filter {
    ...
    protected CompressionFilter(Context ctx, tiledb_filter_type_t filter_type, int level) throws TileDBError {
        super(ctx, filter_type);
        try (NativeArray levelArray = new NativeArray(ctx, new int[] {level,}, Integer.class)) {
            ctx.handleError(tiledb.tiledb_filter_set_option(ctx.getCtxp(), getFilterp(),
              tiledb_filter_option_t.TILEDB_COMPRESSION_LEVEL, levelArray.toVoidPointer()));
