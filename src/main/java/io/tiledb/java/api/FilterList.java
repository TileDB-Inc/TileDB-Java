...
public class FilterList implements AutoCloseable {
    ...
    public FilterList addFilter(Filter filter) throws TileDBError {
        ctx.handleError(tiledb.tiledb_filter_list_add_filter(ctx.getCtxp(), filter_listp, filter.getFilterp()));
        return this;
    }
    ...
    public Filter getFilter(long idx) throws TileDBError {
        ...
        try {
            ctx.handleError(tiledb.tiledb_filter_list_get_filter_from_index(ctx.getCtxp(), filter_listp, idx, _filterpp));
    ...
    public long getNumFilters() throws TileDBError {
        ...
        try {
            ctx.handleError(tiledb.tiledb_filter_list_get_nfilters(ctx.getCtxp(), filter_listp, nfiltersArray.cast()));
    ...
    public FilterList setMaxChunkSize(long chunksize) throws TileDBError {
        ctx.handleError(tiledb.tiledb_filter_list_set_max_chunk_size(ctx.getCtxp(), filter_listp, chunksize));
        return this;
    }
    ...
    public long getMaxChunkSize() throws TileDBError {
        ...
        try {
            ctx.handleError(tiledb.tiledb_filter_list_get_max_chunk_size(ctx.getCtxp(), filter_listp, chunkSizeArray.cast()));
