  ...
  public void createStringVector3(Context ctx, Datatype tiledbType) throws TileDBError {
    ...
    public void testCharType() throws Exception {
        ...
        try (Context ctx = new Context()) {
            ...
            NativeArray a1_data = new NativeArray(ctx, new char[]{'a', 'b', 'c'}, Datatype.TILEDB_CHAR);
            ...
            try (Array array = new Array(ctx, arrayURI, QueryType.TILEDB_READ);
                ...
                query.setBuffer("a1", new NativeArray(ctx, 3 * Long.BYTES, Datatype.TILEDB_UINT64),
                                           new NativeArray(ctx, 3 * Character.BYTES, Datatype.TILEDB_CHAR));
                query.submit();
                char[] resultChars = (char[]) query.getBuffer("a1");
                Assert.assertEquals(chars, resultChars);
            }
        }
    }
}
